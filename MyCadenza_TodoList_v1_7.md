...
- [ ] **MCK_Bewertung.xlsx restrukturieren** — Heutige Tabelle spiegelt 17-Aktionen-Architektur mit Fallback-Logik. Anpassung: alte Zeilen entfernen (`hauptaufgabeErledigt`, `hauptaufgabeUebersprungen`, `aufgabeErstellt`, `aufgabeGeloescht`, `erledigteEntfernt`, `aufgabeZurueckgesetzt`, `exportImport`, `uiAktion`); neue Zeilen einfügen (`.aktionRegistriert`, `.aktionAbgeschlossen`, `.warnungVorAktion`, `.updateVerfuegbar`, `.syncCompleted`, `.syncFailed`); Stamm-/Fallback-Konzept in der Legende obsolet (alle 16 Aktionen sind eigenständig, System-Welt mit eigenen WAVs); pro Welt eigenes Tabellenblatt. *Wird in eigener Folge-Etappe von Claude direkt aufgebaut, nicht von cc.*
